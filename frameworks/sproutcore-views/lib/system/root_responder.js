...
SC.RootResponder = SC.Object.extend(
  ...
  */
  ...
  /**
    ...
    @isReadOnly
  */
  currentWindowSize: Ember.computed.alias('device.currentWindowSize'),

  _beforeResize: function () {
    this._lastWindowSize = this.get('currentWindowSize');
  }.observesBefore('currentWindowSize'),
  ...
  _resize: function() {
    ...
    var newSize = this.get('currentWindowSize'), oldSize = this._lastWindowSize;
  ...
  */
  ...
  computeDesignMode: function () {
    ...
    return this.device.orientation === SC.PORTRAIT_ORIENTATION ? designMode + '_p' : designMode + '_l';
  },
  ...
  */
  ...
  /**
    ...
    invoke the dispatchEvent() method on either the keyPane or on the pane owning
  ...
  */
  dispatchEvent: function(action, evt, target) {
    var pane, ret ;
    ...
    SC.run(this, function() {
      ...
      ret = (pane) ? pane.dispatchEvent(action, evt, target) : null ;
    });
  ...
  */
  ...
  /**
    ...
    particular platform.  Be sure to call this._super().
  ...
  */
  makeTouchResponder: function(touch, responder, shouldStack, upViewChain) {
    ...
    // note that the pane's dispatchEvent function is slightly clever:
    ...
    if (stack.indexOf(responder) < 0) {
      ...
      if (upViewChain) {
        // if we found a valid pane, send the event to it
        try {
          responder = (pane) ? pane.dispatchEvent("touchStart", touch, responder) : null ;
        } catch (e) {
  ...
  */
  _handleModifierChanges: function(evt) {
    ...
    return (changed) ? (this.dispatchEvent('flagsChanged', evt) ? evt.hasCustomEventHandling : YES) : YES ;
  },
  ...
  */
  keydown: function(evt) {
    ...
    if (keyCode===229){
      ...
      return this.dispatchEvent('keyDown', evt);
    }
    ...
    if (this._isFunctionOrNonPrintableKey(evt)) {
      ...
      ret = this.dispatchEvent('keyDown', evt) ;
  ...
  */
  keypress: function(evt) {
    ...
    //there is a special case for arrow key repeating of events in FF.
      ...
      if (isFirefoxArrowKeys || this._isPrintableKey(evt)) {
        return this.dispatchEvent('keyDown', evt) ? evt.hasCustomEventHandling : YES;
    }
    }
  },
  ...
  keyup: function(evt) {
    ...
    return this.dispatchEvent('keyUp', evt) ? evt.hasCustomEventHandling:YES;
  },
  ...
  */
  ...
  mousedown: function(evt) {
    ...
    view = this._mouseDownView = this.dispatchEvent('mouseDown', evt, view) ;
  ...
  */
  mouseup: function(evt) {
    ...
    if (view) {
      handler = this.dispatchEvent('mouseUp', evt, view) ;

      // try doubleClick
      if (!handler && (this._clickCount === 2)) {
        handler = this.dispatchEvent('doubleClick', evt, view) ;
        clickOrDoubleClickDidTrigger = YES;
      }

      // try single click
      if (!handler) {
        handler = this.dispatchEvent('click', evt, view) ;
        clickOrDoubleClickDidTrigger = YES;
      }
    }
    ...
    if (!handler && !clickOrDoubleClickDidTrigger) {
      ...
      if (this._clickCount === 2) {
        handler = this.dispatchEvent('doubleClick', evt, targetView);
      }

      // try singleClick
      if (!handler) {
        handler = this.dispatchEvent('click', evt, targetView) ;
      }
    }
    }
  ...
  */
  ...
  mousewheel: function(evt) {
    var view = this.targetViewForEvent(evt) ,
        handler = this.dispatchEvent('mouseWheel', evt, view) ;
  ...
  */
  ...
  dragenter: function(evt) {
    SC.run(this, function() { this._dragenter(evt); });
  },
  /** @private */
  ...
  selectstart: function(evt) {
    var targetView = this.targetViewForEvent(evt),
        result = this.dispatchEvent('selectStart', evt, targetView);
  ...
  contextmenu: function(evt) {
    ...
    view = this.dispatchEvent('contextMenu', evt, view);
  ...
  animationstart: function (evt) {
      var view = this.targetViewForEvent(evt) ;
      this.dispatchEvent('animationDidStart', evt, view) ;
  ...
  animationiteration: function (evt) {
      var view = this.targetViewForEvent(evt) ;
      this.dispatchEvent('animationDidIterate', evt, view) ;
  ...
  animationend: function (evt) {
      var view = this.targetViewForEvent(evt) ;
      this.dispatchEvent('animationDidEnd', evt, view) ;
