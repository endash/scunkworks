...
SC.CoreView.reopen(
  ...
  toString: function () {
    return "%@ (%@)".fmt(this._super(), this._viewStateString());
  },
  ...
  */
  ...
  _doAttach: function (parentNode, nextNode) {
    ...
    case SC.CoreView.ATTACHED_SHOWN_ANIMATING:
      //@if(debug)
      if (parentNode !== this.get('parentView.layer')) {
  ...
  */
  ...
  _orphaned: function (oldParentView) {
    // Notify.
    if (oldParentView && oldParentView.didRemoveChild) { oldParentView.didRemoveChild(this); }
