...
/** @scope SC.ButtonView.prototype */ {
  ...
  */
  /** @private */
  autoResizeLayer: SC.computed.alias('layer'),

  /** @private */
  autoResizeText: SC.computed.alias('displayTitle'),
  ...
  */
  ...
  init: function() {
    ...
    if (!SC.isNone(this.get('value'))) this._button_valueDidChange();
  },
  ...
  */
  ...
  _runAction: function(evt) {
    ...
    if (action) {
      if (action && (SC.typeOf(action) === SC.T_FUNCTION)) {
        ...
        rootResponder = this.get('pane.rootResponder');
        if (rootResponder) {
  ...
  render: function(context) {
    ...
    var toolTip     = this.get('toolTip'),
      isSelected  = this.get('isSelected') || NO,
      isActive    = this.get('isActive') || NO,
      isDefault   = this.get('isDefault') || NO,
      isCancel    = this.get('isCancel') || NO,
      isToggle    = (this.get('buttonBehavior') === SC.TOGGLE_BEHAVIOR),
      labelId     = SC.guidFor(this) + '-label';
    ...
    context.push(this.get('displayTitle'));
    context = context.end();
  ...
  */
  update: function(jquery) {
    var isToggle = (this.get('buttonBehavior') === SC.TOGGLE_BEHAVIOR),
    ...
    if (this.get('isSegment')) {
      jquery.attr('aria-selected', this.get('isSelected').toString());
    } else if (isToggle) {
      jquery.attr('aria-pressed', this.get('isActive').toString());
    }
    ...
    jquery.toggleClass('icon', !!this.get('icon'));
    jquery.toggleClass('def', !!isDefault);
    jquery.toggleClass('cancel', !!isCancel && !isDefault);

    jquery.find('label').html(this.get('displayTitle'));
  }
}) ;
