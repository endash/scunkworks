...
SC.SelectionSet = SC.Object.extend(SC.Enumerable, SC.Freezable, SC.Copyable,
  ...
  */
  length: function() {
    ...
    if (objects) ret += SC.get(objects, 'length');
    if (sets) sets.forEach(function(s) { ret += SC.get(s, 'length'); });
    return ret ;
  }.property().cacheable(),
  ...
  */
  sources: function() {
    ...
    for(idx=0;idx<len;idx++) {
      set = sets[idx];
      if (set && SC.get(set, 'length')>0 && set.source) ret.push(set.source);
    }
    return ret ;
  }.property().cacheable(),
  ...
  */
  indexSetForSource: function(source) {
    if (!source || !SC.isArray(source)) return null; // nothing to do
    ...
    if (!ret) {
      ...
      if (ret && SC.get(ret, 'length')===0) ret = null;
  ...
  */
  add: function(source, start, length) {
    ...
    if (start === undefined && length === undefined) {
      ...
      if (source instanceof SC.IndexSet) return this.add(source.source, source);
    ...
    oldlen = SC.get(this, 'length');
    setlen = SC.get(set, 'length');
  ...
  */
  remove: function(source, start, length) {
    ...
    if (start === undefined && length === undefined) {
      ...
      if (source instanceof SC.IndexSet) return this.remove(source.source, source);
      if (source instanceof this.constructor) {
        sets = source._sets;
        objects = source._objects;
        len  = sets ? sets.length : 0;
    ...
    set    = this._indexSetForSource(source, YES);
    oldlen = SC.get(this, 'length');
    newlen = oldlen - SC.get(set, 'length');
  ...
  */
  addObjects: function(objects) {
    ...
    oldlen = SC.get(cur, 'length');

    cur.addEach(objects);
    newlen = SC.get(cur, 'length');
  ...
  */
  isEqual: function(obj) {
    ...
    if (SC.get(this, 'length') !== SC.get(obj, 'length')) return NO;

    // check objects
    left = this._objects;
    right = obj._objects;
    if (left || right) {
      if ((left ? SC.get(left, 'length'):0) !== (right ? SC.get(right, 'length'):0)) {
        return NO;
      }
    ...
    sources = SC.get(this, 'sources');
    len     = SC.get(sources, 'length');
    for(idx=0;idx<len;idx++) {
  ...
  */
  freeze: function() {
    if (SC.get(this, 'isFrozen')) { return this ; }
    var sets = this._sets,
...
/** @private */
SC.SelectionSet.reopen({
  clone: function () { return this.constructor.prototype.copy.apply(this) }
});
