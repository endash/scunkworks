...
*/
SC.IndexSet = SC.Object.extend(
  SC.Enumerable, SC.Freezable, SC.Copyable,
/** @scope SC.IndexSet.prototype */ {
  ...
  */
  ...
  init: function () {
    this._super();
    if (!this._content) this._content = [0]
  },
  ...
  */
  rangeStartForIndex: function (index) {
    ...
    SC.assert("index cannot be null or undefined", !Ember.isNone(index))
    ...
    accel = index - (index % this.constructor.HINT_SIZE);
    ret = content[accel];
  ...
  */
  isEqual: function (obj) {
    ...
    if (!obj || !obj instanceof this.constructor || (obj.max !== this.max) || (obj.length !== this.length)) return NO;
  ...
  */
  contains: function (start, length) {
    ...
    if (length === undefined) {
      ...
      } else if (start && start instanceof this.constructor) {
  ...
  */
  without: function (start, length) {
    if (start === this) return this.constructor.create(); // just need empty set
  ...
  */
  add: function (start, length) {
    ...
    if (start && start instanceof this.constructor) {
      content = start._content;
    ...
    if (start === max) {
      ...
      content[max] = 0;
      this.set('max', max);
      this.set('length', this.get('length') + length);
      length = max - start;
  ...
  */
  remove: function (start, length) {
    ...
    if (length === undefined) {
      ...
      } else if (start instanceof this.constructor) {
  ...
  */
  _hint: function (start, length, content) {
    ...
    var skip    = this.constructor.HINT_SIZE,
...
SC.IndexSet.reopenClass({
  create: function() {
    var args = Ember.A(arguments), start, length, ret;
    var ret = this._super();
    if (args.length == 1 && args[0] instanceof this) { start = args[0]; ret.source = start.source; }
    if (args.length == 1 && 'number' === typeof args[0]) { start = args[0]; length = 1; }
    if (args.length == 2 && 'number' === typeof args[0] && 'number' === typeof args[1]) { start = args[0]; length = args[1]; }
...
SC.IndexSet.reopen({
  slice:  function () { return this.constructor.prototype.clone.apply(this) },
  copy: function () { return this.constructor.prototype.clone.apply(this) }
});

SC.IndexSet.EMPTY = SC.IndexSet.create().freeze();
SC.IndexSet.HINT_SIZE = SC.IndexSet.prototype.HINT_SIZE;
