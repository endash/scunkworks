  ...
  test(userAgent, function() {
    ...
    for (var key in expected) {
      ...
      if (browser[key] && typeof browser[key] !== "function") {
        equal(browser[key], expected[key], "'" + key + "' should be '" + expected[key] + "'");
      }
    }
  });
...
test("Language", function(){
  equal(SC.detectBrowser(undefined, 'fr').language, 'fr', "should only show base language part");
  equal(SC.detectBrowser(undefined, 'en-US').language, 'en', "should only show base language part");
  equal(SC.detectBrowser(undefined, 'es-ES').language, 'es', "should only show base language part");
});

test("CountyCode", function(){
  equal(SC.detectBrowser(undefined, 'fr').countryCode, undefined, "should not have a countryCode if the language doesn't indicate it");
  equal(SC.detectBrowser(undefined, 'en-US').countryCode, 'us', "should only show ISO 639-1 countryCode part");
  equal(SC.detectBrowser(undefined, 'es-ES').countryCode, 'es', "should only show ISO 639-1 countryCode part");
});
